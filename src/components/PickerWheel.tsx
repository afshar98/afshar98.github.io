  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-zinc-900 lg:bg-zinc-900/80 border-2 border-green-500/30 rounded-lg p-8 max-w-md w-full mx-4 lg:absolute lg:right-[max(2rem,calc((100vw-56rem)/2-30rem))]"
      >
        <div className="text-green-500 mb-6 font-mono text-center">
          <div className="mb-2">
            Scroll to select • Click or press Enter to choose
          </div>
          <div className="text-zinc-500 text-sm">Press ESC to cancel</div>
        </div>
          ...
          {suggestions.map((suggestion, index) => {
            ...
            return (
              <motion.div
                ...
                animate={{
                  y: offset * 80,
                  opacity: Math.abs(offset) > 1 ? 0 : isSelected ? 1 : 0.3,
                  scale: isSelected ? 1 : 0.8,
                }}
                transition={{ type: "spring", stiffness: 300, damping: 30 }}
                className={`absolute inset-x-0 flex h-20 font-mono ${
                  isSelected ? "text-green-400 text-xl" : "text-green-600"
          ...
          {suggestions.map((_, index) => (
            <div
              ...
              className={`w-2 h-2 rounded-full transition-colors ${
                index === selectedIndex ? "bg-green-500" : "bg-green-900"
              }`}
            />
          ))}
        </div>
